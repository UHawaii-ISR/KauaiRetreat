...
for(buff in buffer){
  ...
  for(level in levels){
    for(trigger in triggers){
      ...
      infrashp <- st_read(sensitivityfolder,layer=paste0("Road_",trigger,level,buff,sensitivity))
      infrasdf <- as.data.frame(infrashp)
...
# add in columns not included in sensitivity analysis (needed for integration with rest of code)
# Define the missing triggers and scenarios
missing_triggers <- c("PF", "WF", "XA")
...
for(trigger in missing_triggers) {
  for(year in years) {
    for(buff in buffers) {
      # Add columns for each infrastructure type
      infrademo[[paste0("hwy_", trigger, "_", year, "_", buff)]] <- NA
      infrademo[[paste0("b_", trigger, "_", year, "_", buff)]] <- NA
      infrademo[[paste0("rd_", trigger, "_", year, "_", buff)]] <- NA
      infrademo[[paste0("rdretreat_", trigger, "_", year, "_", buff)]] <- NA
      infrademo[[paste0("rdbretreat_", trigger, "_", year, "_", buff)]] <- NA
      infrademo[[paste0("seawall_", trigger, "_", year, "_", buff)]] <- NA
    }
  }
}

for(year in years) {
  infrademo[[paste0("hwy_CE_", year, "_")]] <- NA
  infrademo[[paste0("b_CE_", year, "_")]] <- NA
  infrademo[[paste0("rd_CE_", year, "_")]] <- NA
  infrademo[[paste0("rdretreat_CE_", year, "_")]] <- NA
  infrademo[[paste0("rdbretreat_CE_", year, "_")]] <- NA
  infrademo[[paste0("seawall_CE_", year, "_")]] <- NA
}
